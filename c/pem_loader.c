...
int strcpy_safe(char *dest, size_t sizeInBytes, const char *src, size_t count) {
...
int load_pem_key(char *pemstr, size_t pemstr_len, char *password,
  ...
  pub_key = EC_POINT_point2bn(group, pub_key_point, EC_KEY_get_conv_form(eckey),
                              NULL, ctx);
  if (pub_key == NULL) {
  ...
  strcpy_safe(out_priv_key, 64 + 1, priv_key_hex, 64 + 1);
  strcpy_safe(out_pub_key, 130 + 1, pub_key_hex, 130 + 1);
